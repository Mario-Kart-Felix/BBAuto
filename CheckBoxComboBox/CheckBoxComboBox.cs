...
namespace PresentationControls
{
    ...
    public partial class CheckBoxComboBox : PopupComboBox
    {
        ...
        {
            CheckBoxComboBoxItem chbItem = sender as CheckBoxComboBoxItem;
            ...
            if (_first)
            {
                if (chbItem.Text == "(все)")
                {
                    for (int Index = 1; Index <= _CheckBoxComboBoxListControl.Items.Count - 1; Index++)
                    {
                        ...
                        text = string.IsNullOrEmpty(text) ? Item.Text : String.Format(", {0}", Item.Text);

                        if ((Item.Checked) && (text != "(все)"))
                        {
                            _first = false;
                            Item.Checked = false;
                        }
                ...
                {
                    if (_CheckBoxComboBoxListControl.Items[1].Checked)
                    {
                        _CheckBoxComboBoxListControl.Items[1].Checked = false;
                        _first = false;
                    }
                }
            }
